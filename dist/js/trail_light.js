...
function init() {
  ...
  var mesh = new THREE.Mesh(new THREE.PlaneBufferGeometry(size, size, 50, 50), new THREE.MeshBasicMaterial({
    map: feedback.texture
  }));
  scene.add(mesh);
...
container.addEventListener('mousedown', function (event) {
  ...
  touchActive = true;
  feedback.uniforms.uTouchArray.value[0].set(Math.clamp((event.x - offsetX) / size, 0, 1), 1 - Math.clamp((event.y - offsetY) / size, 0, 1));
...
function updateTouchUniforms() {
  ...
  for (var t in touches) {
    touch = touches[t];
    feedback.material.uniforms.uTouchArray.value[i].set(Math.clamp((touch.x - offsetX) / size, 0, 1), 1 - Math.clamp((touch.y - offsetY) / size, 0, 1));
    i++;
  }
